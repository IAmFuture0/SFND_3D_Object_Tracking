...
int main(int argc, const char *argv[])
{
    ...
    vector<string> detectorTypeList{"FAST", "BRISK", "ORB", "AKAZE", "SIFT"}; // "SHITOMASI" {"BRIEF", "FAST", "BRISK", "ORB", "AKAZE", "SIFT"}; 
    vector<string> descriptorTypeList{"BRIEF", "ORB", "FREAK", "AKAZE", "SIFT"}; // "BRISK" {"BRIEF", "ORB", "FREAK", "AKAZE", "SIFT"};
    std::ofstream cameraTTC_file;
    cameraTTC_file.open("cameraTTC.csv");
    cameraTTC_file << "TTC(s),";
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth) {
                cameraTTC_file << "frame " << imgIndex << ',';
    }
    ...
    for(auto detectorType : detectorTypeList){
        for(auto descriptorType : descriptorTypeList){
            ...
            /* create a stream for saving Lidar TTC */
            std::ofstream lidarTTC_file;
            lidarTTC_file.open("lidarTTC.csv");
            lidarTTC_file << "TTC (s) ,";
            
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth) {
                lidarTTC_file << "frame " << imgIndex << ',';
            }
            lidarTTC_file << "\n" << "Lidar ,";
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth) {
                cout << "Detector: " << detectorType << " / Descriptor: " << descriptorType << "| Frame: " << imgIndex << endl;
		        ...
		        // bVis = true; // visualize detected bounding box in each frame
                ...
                if(bVis)
                {
                    show3DObjects((dataBuffer.end()-1)->boundingBoxes, cv::Size(4.0, 20.0), cv::Size(1000, 1000), bVis); // Size(2000, 2000)
                }
                bVis = false;
                ...
                if (dataBuffer.size() > 1) // wait until at least two images have been processed
                {
                    ...
                    for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
                    {
                        ...
                        if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                        {
                            ...
                            /*
                            // Leon -- visualize the matching result
